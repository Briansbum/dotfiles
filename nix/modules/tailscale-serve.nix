...
let
  ...
  serveCmd =
    ...
    let
      httpsPort =
        if svc.path != null then (if svc.tsPort != null then svc.tsPort else 443) else svc.tsPort;
      args =
        if svc.tcp then
          "--bg --tcp=${toString svc.tsPort} localhost:${toString svc.localPort}"
        else if svc.path != null then
          "--bg --https=${toString httpsPort} --set-path=/${removePrefix "/" svc.path} http://localhost:${toString svc.localPort}"
        else
          "--bg --https=${toString httpsPort} http://localhost:${toString svc.localPort}";
      verb = if svc.funnel then "funnel" else "serve";
    in
    "${config.services.tailscale.package}/bin/tailscale ${verb} ${args}";

  afterServices = unique (
    concatMap (svc: optional (svc.afterService != null) "${svc.afterService}.service") (attrValues cfg)
  );
